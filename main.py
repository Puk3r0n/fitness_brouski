...
def start(message):
    ...
    btn1 = types.KeyboardButton("Ввести физ данные")

    markup.add(btn1)
    bot.send_message(message.chat.id,
                     text="Доброго времени суток {0.username}! Я помогу тебе)".format(
...
def func(message):
    ...
    if message.text == "Ввести физ данные":
        phys_data(message)
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        btn1 = types.KeyboardButton("Ввести физ данные")
        btn2 = types.KeyboardButton("Мои параметры")
        btn3 = types.KeyboardButton("Я ввел данные")
        markup.add(btn1, btn2, btn3)
        bot.send_message(message.chat.id, 'главное меню', reply_markup=markup)

    elif message.text == "Калории":
        bot.register_next_step_handler(message, calories, user_info)
...
def phys_data(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    bot.send_message(message.chat.id, text="Мне нужно информации о тебе:", reply_markup=markup)
...
def sex_otvetka(message):
    global user_info
    if message.text == 'мужской' or 'женский':
        a = telebot.types.ReplyKeyboardRemove()
        age_answer = bot.send_message(message.chat.id, text="Какой у тебя возраст?", reply_markup=a)
    user_info = {'sex': message.text, 'id': message.chat.id}
    bot.register_next_step_handler(age_answer, age_otvetka, user_info)
    ...
    try:
        if int(user_info['age']) > 70:
            bot.send_message(message.chat.id,
                             ...
                             reply_markup=markup)
        height_answer = bot.send_message(message.chat.id, text="Какой у тебя рост?", reply_markup=markup)
        bot.register_next_step_handler(height_answer, height_otvetka, user_info)
    except ValueError:
        bot.send_message(message.chat.id, text="Пиши нормально черт(ток числовые значения)", reply_markup=markup)
...
def proverka_vozrasta(user_info):
    if (int(user_info['age']) % 10 == 1) and (int(user_info['age']) != 11) and (int(user_info['age']) != 111):
        year = 'год'
    elif (int(user_info['age']) % 10 > 1) and (int(user_info['age']) % 10 < 5) and (
            int(user_info['age']) != 12) and (
            int(user_info['age']) != 13) and (int(user_info['age']) != 14):
        year = 'года'
    else:
        year = 'лет'
...
def indeks_massi(message):
    try:
        ...
        bot.send_photo(message.chat.id, 'https://shilovo-med.medgis.ru/uploads/3c/bf/d1/dd'
...
def weight_otvetka(message, user_info):
    ...
    try:
        if int(user_info['weight']) > 90:
            bot.send_message(message.chat.id, text="Нам предстоит очень много работы)", reply_markup=markup)
    except ValueError:
        chortik(message)
...
def calories(message, user_info):
    ...
    bot.send_message(message.chat.id, '<b>Выбирете уровень активности</b>\n\n<b>Минимальная активность</b>:\n'
                                      ...
                                      f'креатин!)', reply_markup=markup, parse_mode="html")

    if user_info['sex'] == 'мужской':
        avg = (10 * int(user_info['weight']) + 6.25 * int(user_info['height']) - 5 * int(user_info['age']) + 5)
        user_info['Kcal'] = vivod_calori(message)
    elif user_info['sex'] == 'женский':
        avg = (10 * int(user_info['weight']) + 6.25 * int(user_info['height']) - 5 * int(user_info['age']) - 161)
        user_info['Kcal'] = vivod_calori(message)

    user_data = f'Данные сохранены: \n\n\nПол-> {user_info["sex"]} \nВозраст-> {user_info["age"]}' \
                f' {proverka_vozrasta(user_info)} \nРост-> {user_info["height"]} см\n' \
                f'Вес-> {user_info["weight"]} кг\nKcal-> {user_info["Kcal"]}'
    bot.send_message(message.chat.id, text=f'{user_info}')
